...
import org.mitre.affinity.model.clusters.schema_clusters.ClusterTermVector.ClusterTermStatistics;
 ...
 *
 */
public class SchemaClusterDetailsPane extends Composite {
	
	public SchemaClusterDetailsPane(Composite parent, int style, AffinitySchemaModel affinityModel, ClusterGroup<Integer> cluster) {
		super(parent, style);		
		this.createClusterDetailsPane(affinityModel, cluster);
	}
	
	private void createClusterDetailsPane(final AffinitySchemaModel affinityModel, final ClusterGroup<Integer> cluster) {
		...
		if(affinityModel == null) {
			System.err.println("org.mitre.affinity model is null!");
			return;
		}
		...
		final String numSchemasInCluster = Integer.toString(cluster.getNumClusterObjects());
		final String numSchemas = Integer.toString(affinityModel.getClusters().getObjectIDs().size());		
